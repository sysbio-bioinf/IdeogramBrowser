 ...
 * 
 */
public class IdeogramDB implements Serializable {
	/**
     * 
     */
...
//	private int organism = MOUSE;
	 ...
	 */
	public void clear() {
		tree = new IntervalTree[GlobalConfig.getInstance().getChromosomeCount()];
	...
	public boolean read(Reader reader, IProgressNotifier notifier)
		...
		Label: while (in.ready()) {
			...
			// StringTokenizer(line,"\t",true) didn't work, because it does't
			// count empty entries too.
					...
					try {
						...
						case 2: // band
							...
							if (str.equalsIgnoreCase("0"))
								continue Label;
							if (str.equalsIgnoreCase("ter"))
								continue Label;
							int idx = str.indexOf(".");
							char subsub = str.charAt(str.length() - 1); // the
																		// last
																		// char
																		// in
																		// the
																		// string
							int idx2 = str.length();
							if (idx < 0 && str.length() > 2
									&& Character.isDigit(subsub)) // no point in
																	// str &&
																	// more than
																	// 2 chars
																	// && last
						...
						// default:
						// throw new
						// FileFormatException("Too many columns in line " +
						// in.getLineNumber() );
						}
					} catch (IllegalArgumentException e) {
						throw new FileFormatException("Error in line "
								+ in.getLineNumber() + " field " + (field + 1));
					}
					++field;
				}
			}
			try {
				...
				IntervalTreeNode node = tree[band.chromosome - 1]
						.insert(interval);
				if (node != null) {
					node.content = band;
				}
				...
				throw new FileFormatException(
						"Invalid interval combination in line "
								+ in.getLineNumber());
			}
		}
		return true;
	...
	public void LoadFromFile(String filename) throws IOException,
		...
		if (filename.endsWith(".gz"))
			read(new InputStreamReader(new GZIPInputStream(new FileInputStream(
					filename))));
		else
