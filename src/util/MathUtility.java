...
public class MathUtility
{
    ...
    */
	...
	public static double maxDouble( Iterator iterator )
	{
	    boolean first = true;
	    double mx = Double.NaN, val;
	    
	    while( iterator.hasNext() )
	    {
	        val = ((Double)iterator.next()).doubleValue();
	        if( ! Double.isNaN(val) && ( first || val > mx ) )
	        {
	            first = false;
	            mx = val;
	        }
    ...
    public static double minDouble( Iterator iterator ) 
    {
	    boolean first = true;
	    double mx = Double.NaN, val;
	...
	public static <T> T quantile( Vector<T> array, Comparator<T> c, float q )
    {
    	q = MathUtility.restrict( q, 0.0f, 1.0f);
    	
    	Vector<T> ar = (Vector<T>)array.clone();
    	
    	Collections.sort( ar, c );
    	
		return ar.get((int)(Math.floor(((float)(ar.size()-1))*q)));
    }
    ...
    public static double quantile( double[] array, double q )
    {
    	q = MathUtility.restrict( q, 0.0f, 1.0f);
    	Arrays.sort(array);
		return array[((int)(Math.floor(((double)(array.length-1))*q)))];
    }
